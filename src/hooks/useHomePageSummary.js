  ...
  useMemo(() => {
    ...
    const filledSlots = getWeekRecipeIds(
      weeklyMenu.week.map((day) =>
        Object.fromEntries(PLANNED_MEAL_KEYS.map(({ key }) => [key, day?.[key]])),
      ),
    );
    ...
    const completion = Math.round(
      (occupiedSlotCount / (DAYS_IN_WEEK * PLANNED_MEAL_KEYS.length)) * 100,
    );
    const repeatedRecipes = [
      ...filledSlots
        .reduce((counts, recipeId) => {
    ...
    const lunchRecipeIds = weeklyMenu.week.flatMap((day) => getSlotRecipeIds(day?.lunch));
    ...
    const shoppingSummary = getShoppingListSummary(
      weeklyMenu.shopping,
      weeklyMenu.week,
      recipeList,
    );
